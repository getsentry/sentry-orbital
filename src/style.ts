...
export const DEFAULT_STYLE: GlobeStyle = {
  ...
  effects: {
    ...
    vignette: { enabled: true, darkness: 0.55, offset: 0.26 },
    chromaticAberration: { enabled: true, offset: 0.002 },
    noise: { enabled: true, opacity: 0.26 },
    ...
    brightnessContrast: { enabled: true, brightness: -0.09, contrast: -0.03 },
 ...
 *  Saved from the style panel, so these are full objects rather than diffs:
 *  `merge` only descends one level, and a preset that leans on DEFAULT_STYLE for
 *  its nested blocks would drift the moment those defaults change. */
export const PRESETS: Record<string, GlobeStyle> = {
  "CRT 1": merge(DEFAULT_STYLE, {
    ...
    dots: {
      visible: false, count: 34000, size: 0.008, spacing: 1, color: "#7c8db0",
      colorB: "#38e1f2", colorMode: "flat", opacity: 1, altitude: 0.004, shape: "circle",
      sizeAttenuation: true, additive: false, jitter: 0,
    },
    atmosphere: { enabled: true, color: "#73c0ff", intensity: 0.09, power: 2.9, scale: 1.14 },
    lights: {
      followCamera: false, showHelper: false,
      ambient: { enabled: true, color: "#ffffff", intensity: 0.4 },
      hemisphere: { enabled: false, sky: "#88bbff", ground: "#221133", intensity: 0.6 },
      directional: { enabled: false, color: "#bcd6ff", intensity: 1, x: 4, y: 2, z: 3, castShadow: false },
      point: { enabled: false, color: "#ffffff", intensity: 1, x: 2, y: 2, z: 2, distance: 0, decay: 2 },
    },
    shadows: {
      enabled: false, type: "pcfsoft", bias: -0.0005, normalBias: 0.02, radius: 2,
      mapSize: 2048,
    },
    beams: {
      visible: true, style: "dot", colorMode: "sdk", color: "#ffb347", length: 0.1,
      width: 0.006, depthRatio: 1, faceShade: 0.35, hideEndOn: 0.75, widthTaper: 0.6,
      fade: "linear", riseSeconds: 1, riseEase: "easeOut", holdSeconds: 0.4,
      shrinkSeconds: 1.4, shrinkEase: "linear", trail: 1, offset: 0, flicker: 0,
      flickerSpeed: 8, spin: 0, softness: 0.5, direction: "out", jitterHue: 0, jitterLength: 0,
      jitterLife: 0, lengthByIntensity: 0.14, size: 0.039, opacity: 0.8, additive: false,
      dimFactor: 0.03, brightness: 2.35, grow: 1,
    },
    effects: {
      toneMappingExposure: 1,
      bloom: { enabled: false, intensity: 1, threshold: 0.6, smoothing: 0.3, radius: 0.6, mipmap: false },
      ...
      noise: { enabled: true, opacity: 0.29 }, pixelation: { enabled: true, granularity: 10 },
      scanline: { enabled: false, density: 1.25 },
      dotScreen: { enabled: false, scale: 1, angle: 1.57 },
      hueSaturation: { enabled: false, hue: 0, saturation: 0 },
      brightnessContrast: { enabled: false, brightness: 0, contrast: 0 },
      sepia: { enabled: false, intensity: 1 }, grayscale: { enabled: false },
      posterize: { enabled: false, bits: 5 }, ascii: { enabled: false, cellSize: 16 },
      glitch: { enabled: false, strength: 0.3 },
      tiltShift: { enabled: true, blur: 0.98, focusArea: 1 },
    },
    stars: {
      enabled: false, color: "#ffffff", count: 2600, radius: 120, depth: 60, factor: 3.5,
      saturation: 0, fade: true, speed: 0.3,
    },
    clouds: {
      enabled: false, style: "soft", color: "#ffffff", opacity: 0.42, altitude: 0.022,
      drift: 0.012, seed: 7, lit: false, additive: false, coverage: 0.45, softness: 0.28,
      scale: 3.2, detail: 4, clusters: 34, blockSize: 0.026, spread: 2.6, puffiness: 3,
      flatness: 0.45, ragged: 0.5, sizeVariance: 0.75, speedVariance: 0.6, gap: 0, shade: 0.8,
      tint: 0.12, castShadow: false,
    },
    ufo: { enabled: true, color: "#ff2bd6", size: 0.075, altitude: 0.34, speed: 0.33 },
    halo: {
      enabled: false, color: "#6d5bd0", count: 9000, radius: 1.45, thickness: 0.12, size: 0.012,
      opacity: 0.5, additive: true,
    },
    camera: {
      fov: 40, flatten: 0.55, distance: 3.6, tilt: 0.6,
    },
  }),
  ...
  "CRT 2": merge(DEFAULT_STYLE, {
    background: { color: "#000b31" },
    globe: {
      visible: true, mode: "solid", material: "basic", color: "#4600ff", landColor: "#73c792",
      emissive: "#000000", emissiveIntensity: 0, roughness: 1, metalness: 0, clearcoat: 0,
      clearcoatRoughness: 0.3, opacity: 1, wireframe: false, flatShading: false,
      radiusScale: 1.003, segments: 140, autoRotate: true, rotateSpeed: 0.04, relief: 0.03,
      edgeSoftness: 0, reliefSoftness: 4, reliefBevel: 0.85, receiveShadow: false,
    },
    dots: {
      visible: false, count: 34000, size: 0.008, spacing: 1, color: "#7c8db0",
      colorB: "#38e1f2", colorMode: "flat", opacity: 1, altitude: 0.004, shape: "circle",
    ...
    beams: {
      visible: true, style: "line", colorMode: "sdk", color: "#ffb866", length: 0.1,
      width: 0.0175, depthRatio: 1.1, faceShade: 1, hideEndOn: 0.75, widthTaper: 0,
      fade: "easeOut", riseSeconds: 0.25, riseEase: "easeOut", holdSeconds: 20,
      shrinkSeconds: 0.25, shrinkEase: "easeOut", trail: 1, offset: 0, flicker: 0,
      flickerSpeed: 8, spin: 0.65, softness: 1, direction: "out", jitterHue: 0, jitterLength: 0,
      jitterLife: 0, lengthByIntensity: 0, size: 0.039, opacity: 1, additive: false,
      dimFactor: 0.01, brightness: 0.7, grow: 1,
    },
    effects: {
      toneMappingExposure: 1,
      bloom: { enabled: false, intensity: 1, threshold: 0.6, smoothing: 0.3, radius: 0.6, mipmap: false },
      vignette: { enabled: true, darkness: 0.55, offset: 0.26 },
      chromaticAberration: { enabled: true, offset: 0.002 },
      noise: { enabled: true, opacity: 0.26 }, pixelation: { enabled: true, granularity: 5 },
      scanline: { enabled: false, density: 1.25 },
      dotScreen: { enabled: false, scale: 1, angle: 1.57 },
      hueSaturation: { enabled: false, hue: 0, saturation: 0 },
      brightnessContrast: { enabled: true, brightness: -0.09, contrast: -0.03 },
    ...
    stars: {
      enabled: true, color: "#5c5ba3", count: 2900, radius: 20, depth: 200, factor: 7.8,
      saturation: 0, fade: true, speed: 0.1,
    },
    clouds: {
      enabled: true, style: "blocky", color: "#ffffff", opacity: 0.69, altitude: 0.002,
      drift: 0.034, seed: 630, lit: false, additive: false, coverage: 0.45, softness: 0.28,
      scale: 3.2, detail: 4, clusters: 36, blockSize: 0.023, spread: 1.5, puffiness: 3,
      flatness: 0.15, ragged: 0.67, sizeVariance: 1, speedVariance: 1, gap: 0, shade: 0.8,
      tint: 0.32, castShadow: false,
    },
    ufo: { enabled: true, color: "#ff2bd6", size: 0.06, altitude: 0.12, speed: 0.06 },
    halo: {
      enabled: false, color: "#6d5bd0", count: 9000, radius: 1.45, thickness: 0.12, size: 0.012,
      opacity: 0.5, additive: true,
    },
    camera: {
      fov: 40, flatten: 0.55, distance: 3.6, tilt: 1.35,
    },
  }),
};
